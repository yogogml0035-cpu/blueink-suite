...
**状态层回归检查**（`state-layer` / `explicit-entry` / `stage-boundaries`）验的是那些"破掉之后没人会立刻发现"的边界。它们不是自证式的字符串断言——`state-layer` 会在临时目录里真的绑定、索引、检索、校验 URL、跑记忆升降、审计和附件登记，共 174 项。
...
| `case-12` | **从一次真实运行脱敏而来**：附件驱动、零轮访谈、各阶段读上游回执、走到编辑红队尚未交付 | `incomplete`（在飞，不是违约） | **test（保留测试）** |
...
`case-12` 与其它构造夹具不同：**它是从一次真实运行脱敏来的**（路径换成占位符、正文压成一行，回执结构原样保留）。它存在的理由是一次真实教训：构造夹具全绿的同时，真实运行第一次审计抓出了三个误报，其中一个会让 A2 在**每一次合规运行**上判违约——因为构造夹具里的路径形态只有"相对 kb_root"，一条运行目录内的引用都没有。**构造的输入验不出作者没想到的形态。** 它设为保留测试，不参与日常评分，以免被当成优化目标。
...
```json
{
  ...
  "criteria": [
    ...
    {"id": "explicit-entry", "text": "入口机械关闭模型自动调用，且 SKILL.md 与 command 两处一致", "type": "command",
