...
| `case-10` | 来源清单里引用了一个只在检索候选里出现、从没被任何角色打开过的文件 | `violated` A5 | val |
| `case-11` | 取证报了两组口径冲突，写作程序既没有 `assumptions` 也没有决策卡 | `pass` + A5 携带静默选边提示 | val |
| `case-12` | **从一次真实运行脱敏而来**：附件驱动、零轮访谈、各角色读上游回执、走到编辑反方尚未交付 | `incomplete`（在飞，不是违约） | **test（保留测试）** |
...
- **文案质量没有自动证据。** 审计器守的是形式契约；"六个角色协作出来的稿子是否真的更好"必须由文案老师在真实任务上盲评。`case-12` 从一次真实运行脱敏而来，设为保留测试，不参与日常评分——**构造的夹具验不出作者没想到的形态**。
- **真实端到端的文案质量没有被证明。** "六个角色协作出来的稿子是否更好"必须由文案老师用真实业务任务盲评。
...
```json
{
  ...
  "criteria": [
    ...
    {"id": "role-boundaries", "text": "技能本体不含品牌语料、不含本机绝对路径、无越界执行与外发", "type": "command",
