...
这份评测只验证 BlueInk 的确定性状态层和五项审计合同，不评文案质量、不评模型表达，也不模拟真实 Claude Code 对话。
...
`run` 命令对每个 golden 运行同一个审计器。golden 是人工确认过的当前运行记录夹具；`expected.json` 是审计器应该给出的独立结论。基线比较验证 A1—A5 的状态、失败项、定位证据和缺失产物，而不是验证某一段文案。
...
| A3 | 动态访谈 | Schema 5 生成任务至少一轮、一次只问一个问题，最后一轮确认方向且得到老师确认 |
| A4 | 阶段边界 | 当前运行只使用 Schema 5 产物，不生成 `draft.md` 等旧阶段文件，核验角色和方向记录合法 |
| A5 | 输出有效 | `delivery.md`、`verify.json`、`delivery-check.md` 的结论和来源结构一致；未归档缺侧车只能判 `incomplete` |

`schema4` 夹具只验证历史运行仍可被审计；它不代表新运行可以继续写入旧产物。新运行的唯一正文是 `delivery.md`，交付核对信息单独位于 `delivery-check.md`。
...
评测规格内的 5 个 command criteria 只检查审计结论自身的 schema、自洽性、可定位性、解释完整性和五项合同名称。`--rollout` 另外把实际审计输出与每个 golden 的 `expected.json` 做 JSON 值比较，因此 A1—A5 的具体判定也会回归。

当前规模为 8 个夹具 × 5 项命令检查，共 40 项；其中日常评分跳过 1 个 holdout，发布前再完整执行。

状态层和自证门是独立发布门，不对每个 golden 重复执行：

```bash
python3 scripts/test_state.py                       # 当前状态层集成回归，201 项
...
```json
{
  ...
  "criteria": [
    {"id": "schema", "text": "审计结论结构完整，五项契约一条不少", "type": "command", "cmd": "python3 scripts/blueink.py check-verdict {output} --check schema"},
    ...
    {"id": "contracts", "text": "五项合同名称未被改名或替换", "type": "command", "cmd": "python3 scripts/blueink.py check-verdict {output} --check contracts"}
  ],
  "golden": [
