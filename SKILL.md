...
description: >-
  ...
  媒体观点供稿、活动邀请函、媒体传播指引、核心信息、演讲稿、QA、社会化文案、视频脚本和主持人串词。必须显式调用 /blueink-suite。
license: proprietary
...
`index` 会报告按品类取风格参考的可达性。出现「按品类取风格参考：N 个品类里 0 个能取到」时，说明这个库的目录名和默认骨架不同，证据类型只能靠文件名关键词猜——此时**只问老师一个问题**：已发表的成品稿件放在哪几个目录。拿到答案后重新绑定并重建索引，不要改判定逻辑，也不要放宽检索：

```bash
$BLUEINK bind --force --brand "<品牌>" --kb "<原路径>" --layout 成品参考=<目录1>,<目录2>
$BLUEINK index --full
```

`--layout 标签=目录` 可重复，声明过的目录整目录直接定证据类型，不再靠文件名猜。同一标签可以对多个目录，用逗号分隔或重复传参——已发表成品散在多个目录时必须都写上。老师说不清或不确定时保持现状，改用 `retrieve --under` 按老师当次点名的目录取材。`doctor` 也会报同一项可达性。
...
| 记忆库有可用候选、需要决定本次用不用 | 无需额外文件；按 `generate.md`「展示写法之前先读一次条件化记忆」执行 |
...
负责文案生成与来源核对；不做 Word 排版、字体、配图、品牌文档模板、改稿编排或版本回退。反馈只形成有条件的候选证据，不自动改方法论或源知识库——但生成前必须读一次这些候选（`memory list --for-generation`），并在方向记录里如实交代过目结果；写进去读不出来等于没有反馈回路。
