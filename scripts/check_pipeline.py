...
def _stdlib_module_names() -> frozenset[str]:
    """Top-level stdlib module names, on 3.9 too.

    `sys.stdlib_module_names` only exists from 3.10. This skill declares a 3.9
    floor, so reading the attribute directly made the verifier crash on exactly
    the interpreter it promised to support -- and because this script was not
    wired into any quality gate, nothing caught it. The 3.9 fallback enumerates
    the stdlib directory instead: top-level `*.py` modules, packages with an
    `__init__.py`, and compiled extensions under `lib-dynload`. Deliberately
    NOT `site-packages` -- that is where third-party code lives, and treating it
    as stdlib would turn this check into a rubber stamp.
    """
    ...
    for key in ("stdlib", "platstdlib"):
        ...
        for entry in base.iterdir():
            if entry.name.startswith("_") and entry.name != "__future__":
                found.add(entry.name.split(".")[0].lstrip("_"))
            if entry.is_file() and entry.suffix == ".py":
                found.add(entry.stem)
            elif entry.is_dir():
                if entry.name == "lib-dynload":
                    found |= {p.name.split(".")[0] for p in entry.iterdir() if p.is_file()}
